...
class SV_Avulsos
{
  ...
  {
    $args = (new SV_Depoimento($id))->get_post_metas_values();
  ...
  {
    $solucoes = new SV_Solucoes(false);
  ...
  public function render_section_big_numbers_associacoes(): string
  {
    ...
    if ($query->have_posts()) {
      while ($query->have_posts()) {
        $query->the_post();
        $meta = (new SV_Associacao(get_the_ID()))->get_post_metas_values();
