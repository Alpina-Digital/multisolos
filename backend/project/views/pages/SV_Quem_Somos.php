 ...
 * @version 4.0
 */
class SV_Quem_Somos extends Alp_Page
{
  use SV_Section_Projetos;
   ...
   * @see SV_Quem_Somos::render_wrapper_nos_destacamos_dna()
   ...
   * @see SV_Quem_Somos::render_section_nos_destacamos()
   */
   ...
   * @see SV_Avulsos::render_wrapper_branco_por_cima()
   * @return string HTML da seção.
   */
   ...
   * @see SV_Quem_Somos::render_wrapper_onde_estamos_servicos_ponta()
   * @return string HTML da seção.
   */
  ...
  private function render_section_associacoes(): string
  {
    ...
    if ($query->have_posts()) {
      while ($query->have_posts()) {
        $query->the_post();
        $associacao = new SV_Associacao(get_the_ID());
 ...
 * Hooks
 */
add_action('after_setup_theme', [new SV_Quem_Somos(), 'setup']);
