...
/**
 * Reproduz uma seção com um Grid de Etapas do Serviço.
 * @package Alpina V4
 * @version 4.0
 */
trait MS_Etapas_Servico
{
  use Alp_Renderable;

  /**
   * Adiciona metaboxes para a seção de etapas do servico.
   ...
   * @return Alp_Metabox
   */
  public function chain_metaboxes_etapas_servico(Alp_Metabox $metaboxes, bool $texto_item = true): Alp_Metabox
  {
    $metaboxes
      ->add_metabox_box('etapas', 'Etapas de Serviço')
  ...
  /**
   * Renderiza a seção de Etapas do Serviço.
   * @param string $titulo Título da etapa.
   * @return string HTML renderizado.
   */
  public function render_section_etapas_servico(bool $texto_itens = false, string $classes = 'padding-y-xl'): string
  {
    ...
    foreach ($args['itens'] as $i => &$item) {
      $item = $this->render_item_etapa_servico($item, $i + 1, $texto_itens ? 'v2' : 'v1');
    }

    $args['itens'] = implode('', $args['itens']);

    return $this->html('frontend/views/avulsos/etapas/section-etapas-servico', $args);
  }

  public function render_item_etapa_servico(array $item, int $numero, string $v): string
  {
    $item['numero'] = $numero;
    return $this->html("frontend/views/avulsos/etapas/item-etapa-servico-{$v}", $item);
  }
}
